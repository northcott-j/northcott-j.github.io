...
function luc_add_pages()
  {
	  ...
	  // Create table if it doesn't exist
       if (($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) or ($AnalyticStats_Option['AnalyticStats_DB_Version'] <> AnalyticStats_DB_VERSION))
      ...
      $mincap = $AnalyticStats_Option['AnalyticStats_MinPermit'];
      if ($mincap == '')
          $mincap = 'switch_themes';
		  
	
	if (isset($_GET['AnalyticStats-action']))
	{	 
		if ($_GET['AnalyticStats-action'] == 'updategeoipdat')
			luc_GeoIP_update_db('country');
		if ($_GET['AnalyticStats-action'] == 'updategeoipcitydat')
			luc_GeoIP_update_db('city');
	}
	
	  // add submenu
      add_menu_page('Stats', 'Stats', $mincap, __FILE__, 'luc_main',AnalyticStats_PLUGIN_URL.'/images/stat.png');
      add_submenu_page(__FILE__, __('Visitor Spy', 'AnalyticStats'), __('Visitor Spy', 'AnalyticStats'), $mincap,'AnalyticStats/action=spyvisitors', 'luc_spy_visitors');
	  if ($AnalyticStats_Option['AnalyticStats_Dont_Collect_Spider'] == '')
	  add_submenu_page(__FILE__, __('Bot Spy', 'AnalyticStats'), __('Bot Spy', 'AnalyticStats'), $mincap,'AnalyticStats/action=spybot', 'luc_spy_bot');
	  add_submenu_page(__FILE__, __('Yesterday ', 'AnalyticStats'), __('Yesterday ', 'AnalyticStats'), $mincap,'AnalyticStats/action=yesterday', 'luc_yesterday');
	  add_submenu_page(__FILE__, __('Referrer', 'AnalyticStats'), __('Referrer', 'AnalyticStats'), $mincap, 'AnalyticStats/action=referrer', 'luc_AnalyticStats_referrer');
	  add_submenu_page(__FILE__, __('URL Monitoring', 'AnalyticStats'), __('URL Monitoring', 'AnalyticStats'), $mincap, 'AnalyticStats/action=urlmonitoring', 'luc_AnalyticStats_url_monitoring');
	  add_submenu_page(__FILE__, __('Statistics', 'AnalyticStats'), __('Statistics', 'AnalyticStats'), $mincap,'AnalyticStats/action=details','luc_statistics');
      add_submenu_page(__FILE__, __('Options', 'AnalyticStats'), __('Options', 'AnalyticStats'), $mincap,'AnalyticStats/action=options', 'luc_Options');
  }
  
function luc_AnalyticStats_load_time()
{
	echo "<font size='1'><br>Stats page generated in " . timer_stop(0,2) . " seconds with ".get_num_queries()." SQL queries.</font>";
}
  
function AnalyticStats_admin_init()
{
	global $wpdb,$AnalyticStats_Option;
	...
	// jQuery Pagination
	//wp_enqueue_script('jquery-pagination', AnalyticStats_PLUGIN_URL . '/js/jquery.pagination.js', array ('jquery', 'jquery-ui-core'));
	
	//  Add AJAX support
	wp_enqueue_script('luc-ajax-geoip', AnalyticStats_PLUGIN_URL . '/js/luc.ajax.geoip.js', array ('jquery'));
	//wp_enqueue_script('luc-ajax-pagination', AnalyticStats_PLUGIN_URL . '/js/luc.ajax.pagination.js', array ('jquery'));
	wp_enqueue_script('luc-ajax-tables', AnalyticStats_PLUGIN_URL . '/js/luc.ajax.tables.js', array ('jquery'));
	
	// jQuery CSS
	wp_enqueue_style('wp_customtextCSS', AnalyticStats_PLUGIN_URL . '/css/style.css');
	wp_enqueue_style('jquery.ui.theme', AnalyticStats_PLUGIN_URL . '/css/jquery-ui-1.8.16.custom.css');
	wp_enqueue_style('jquery.pagination.theme', AnalyticStats_PLUGIN_URL . '/css/pagination.css');
	wp_enqueue_style('jquery.overrides', AnalyticStats_PLUGIN_URL . '/css/jquery.override.css');
	
	//  Add AJAX support
	//wp_enqueue_script('my-ajax-request', AnalyticStats_PLUGIN_URL . '/js/ajax.js', array ('jquery'));
...
function luc_Options()
  {  	global $wpdb;
      ...
      {   // General Tab
		...
		$AnalyticStats_Option['AnalyticStats_Use_GeoIP'] = (isset($_POST['AnalyticStats_Use_GeoIP']) ? $_POST['AnalyticStats_Use_GeoIP'] : '');
		$AnalyticStats_Option['AnalyticStats_MinPermit'] = (isset($_POST['AnalyticStats_MinPermit']) ? $_POST['AnalyticStats_MinPermit'] : $AnalyticStats_Option['AnalyticStats_MinPermit']);

		// Data Collection and Retention
		$AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] = (isset($_POST['AnalyticStats_Dont_Collect_Logged_User']) ? $_POST['AnalyticStats_Dont_Collect_Logged_User'] : '');
		$AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User_MinPermit'] = (isset($_POST['AnalyticStats_Dont_Collect_Logged_User_MinPermit']) ? $_POST['AnalyticStats_Dont_Collect_Logged_User_MinPermit'] : '');
		$AnalyticStats_Option['AnalyticStats_Dont_Collect_Spider'] = (isset($_POST['AnalyticStats_Dont_Collect_Spider']) ? $_POST['AnalyticStats_Dont_Collect_Spider'] : '');

		// Pages Options
		$AnalyticStats_Option['AnalyticStats_SpyVisitor_IP_Per_Page'] = (isset($_POST['AnalyticStats_SpyVisitor_IP_Per_Page']) ? $_POST['AnalyticStats_SpyVisitor_IP_Per_Page'] : $AnalyticStats_Option['AnalyticStats_SpyVisitor_IP_Per_Page']);
		$AnalyticStats_Option['AnalyticStats_SpyVisitor_Visits_Per_IP'] = (isset($_POST['AnalyticStats_SpyVisitor_Visits_Per_IP']) ? $_POST['AnalyticStats_SpyVisitor_Visits_Per_IP'] : $AnalyticStats_Option['AnalyticStats_SpyVisitor_Visits_Per_IP']);
		$AnalyticStats_Option['AnalyticStats_Rows_Per_Latest'] = (isset($_POST['AnalyticStats_Rows_Per_Latest']) ? $_POST['AnalyticStats_Rows_Per_Latest'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
		$AnalyticStats_Option['AnalyticStats_Show_Browser_name']= (isset($_POST['AnalyticStats_Show_Browser_name']) ? $_POST['AnalyticStats_Show_Browser_name'] : '');
		$AnalyticStats_Option['AnalyticStats_Show_OS_name']= (isset($_POST['AnalyticStats_Show_OS_name']) ? $_POST['AnalyticStats_Show_OS_name'] : '');
		$AnalyticStats_Option['AnalyticStats_Show_domain_name']= (isset($_POST['AnalyticStats_Show_domain_name']) ? $_POST['AnalyticStats_Show_domain_name'] : '');
		$AnalyticStats_Option['AnalyticStats_locate_IP']= (isset($_POST['AnalyticStats_locate_IP']) ? $_POST['AnalyticStats_locate_IP'] : $AnalyticStats_Option['AnalyticStats_locate_IP']);
		
		update_option('AnalyticStats_Option', $AnalyticStats_Option);
        echo "<br /><div class='wrap' align ='center'><h2>" . __('Recorded !', 'AnalyticStats') . "<h2>
		  <IMG style='border:0px;width:20px;height:20px;' SRC='".AnalyticStats_PLUGIN_URL."/images/ok.gif'></div>";
...
<!-- tab 1 -->
			...
			<tbody>		
		<tr><td colspan='2'><input type=checkbox name='AnalyticStats_Dont_Collect_Logged_User' id='AnalyticStats_Dont_Collect_Logged_User' value='checked' 
		<?php echo $AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] ?> >
		<label for='AnalyticStats_Dont_Collect_Logged_User'>Do not collect data about logged users who have at least these capability</label>
		<select name="AnalyticStats_Dont_Collect_Logged_User_MinPermit"><?php luc_dropdown_caps($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User_MinPermit']); ?></select>
		<a href="http://codex.wordpress.org/Roles_and_Capabilities" target='_blank'>more info</a><br>
		<input type=checkbox name='AnalyticStats_Dont_Collect_Spider' id='AnalyticStats_Dont_Collect_Spider' value='checked' 
		<?php echo $AnalyticStats_Option['AnalyticStats_Dont_Collect_Spider'] ?> >
		<label for='AnalyticStats_Dont_Collect_Spider'>Do not collect spiders visits</label></td><td></td></tr>
		...
		<tbody>
		  ...
		  <td><select name="AnalyticStats_MinPermit"><?php luc_dropdown_caps($AnalyticStats_Option['AnalyticStats_MinPermit']); ?></select>
		 ...
		 <thead><tr>
			...
			<tbody>		
		<tr><td colspan='2'> 
		<input type=checkbox name='AnalyticStats_Show_OS_name' id='AnalyticStats_Show_OS_name' value='checked' 
		<?php echo $AnalyticStats_Option['AnalyticStats_Show_OS_name'] ?> >
		<label for='AnalyticStats_Show_OS_name'>Show the name of OS</label>
		<br>
		<input type=checkbox name='AnalyticStats_Show_Browser_name' id='AnalyticStats_Show_Browser_name' value='checked' 
		<?php echo $AnalyticStats_Option['AnalyticStats_Show_Browser_name'] ?> >
		<label for='AnalyticStats_Show_Browser_name'>Show the name of browsers</label>
		</td><td></td></tr>
					<tr>
						 <td >'Latest' Reports : Default number of rows</td>
						 <td>
						 <select name="AnalyticStats_Rows_Per_Latest">
								<option value="5"
								<?php if ($AnalyticStats_Option['AnalyticStats_Rows_Per_Latest'] == 5)
									echo "selected"; ?>>5</option>
								<option value="10"
								<?php if ($AnalyticStats_Option['AnalyticStats_Rows_Per_Latest'] == 10)
									echo "selected"; ?>>10</option>
								<option value="25"
								<?php if ($AnalyticStats_Option['AnalyticStats_Rows_Per_Latest'] == 25)
									echo "selected"; ?>>25</option>
								<option value="50"
								<?php if ($AnalyticStats_Option['AnalyticStats_Rows_Per_Latest'] == 50)
									echo "selected"; ?>>50</option>
								<option value="100"
								<?php if ($AnalyticStats_Option['AnalyticStats_Rows_Per_Latest'] == 100)
		  ...
		  <select name="AnalyticStats_SpyVisitor_IP_Per_Page">
		      <option value="20" <?php if ($AnalyticStats_Option['AnalyticStats_SpyVisitor_IP_Per_Page'] == 20) 
		                                   echo "selected"; ?>>20</option>
			  <option value="50" <?php if ($AnalyticStats_Option['AnalyticStats_SpyVisitor_IP_Per_Page'] == 50) 
			                               echo "selected"; ?>>50</option>
              <option value="100" <?php if ($AnalyticStats_Option['AnalyticStats_SpyVisitor_IP_Per_Page'] == 100)
                                           echo "selected"; ?>>100</option>
         </select>
		 <br>
		 Visits per visitor&nbsp;&nbsp;
         <select name="AnalyticStats_SpyVisitor_Visits_Per_IP">
		      <option value="20" <?php if ($AnalyticStats_Option['AnalyticStats_SpyVisitor_Visits_Per_IP'] == 20)
                                           echo "selected"; ?>>20</option>
              <option value="50" <?php if ($AnalyticStats_Option['AnalyticStats_SpyVisitor_Visits_Per_IP'] == 50)
                                           echo "selected"; ?>>50</option>
              <option value="100" <?php if ($AnalyticStats_Option['AnalyticStats_SpyVisitor_Visits_Per_IP'] == 100)
                                           echo "selected"; ?>>100</option>
         </select>
		 </td><td></td><td></td></tr>
		 </tbody>	
...
<!-- tab 2 -->
		 ...
		 <?php // Use GeoIP? http://geolite.maxmind.com/download/geoip/api/php/
						if (($AnalyticStats_Option['AnalyticStats_Use_GeoIP'] == 'checked')
							AND (!function_exists('geoip_open')))
							include AnalyticStats_PLUGIN_PATH .'/GeoIP/geoipcity.inc';
						luc_GeoIP(); 
					?>
			</div>
			</div><!-- tabbed -->
         ...
         <input type=hidden name=page value=AnalyticStats>
		 <input type=hidden name=AnalyticStats-action value=options>
...
function luc_AnalyticStats_SearchQ($surl)
{
	$url = parse_url($surl);
...
function luc_main_table_latest_hits()
{
	...
	global $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['hitsrows']) ? $_POST['hitsrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
	?>
	<table class='widefat'>
		<thead>
		<tr>
		...
		<?php if (($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] != 'checked') or (($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] == 'checked')and (current_user_can($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User_MinPermit'])))) echo "<th scope='col'>User</th>"; ?>
	...
	$text_OS = ($AnalyticStats_Option['AnalyticStats_Show_OS_name']=='checked') ? true : false;
	$text_browser =($AnalyticStats_Option['AnalyticStats_Show_Browser_name']=='checked' )? true : false;
	...
	foreach ($rks as $rk)
	{
		...
		if (($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] != 'checked') or (($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] == 'checked')and (current_user_can($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User_MinPermit']))))
...
function luc_main_table_latest_search()
{
	...
	global $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['searchrows']) ? $_POST['searchrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
	?>
	<table class='widefat' >
		<thead>
		<tr>
...
function luc_main_table_latest_referrers()
{
	...
	global $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['referrersrows']) ? $_POST['referrersrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
	?>
	<table class='widefat' width="100%" >
		...
		<tr>
			...
			<th scope='col'>Time</th>
			<th scope='col'>IP</th>
		<?php if ($AnalyticStats_Option['AnalyticStats_Show_domain_name']=='checked') echo "<th scope='col'>Domain</th>" ?>
			<th scope='col'>Language</th>
			<th scope='col'>Country</th>
...
function luc_main_table_latest_feeds()
{
	...
	global $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['feedsrows']) ? $_POST['feedsrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
	?>
	<table class='widefat' >
		<thead>
		<tr>
	...
	if ($AnalyticStats_Option['AnalyticStats_Dont_Collect_Logged_User'] != 'checked')
...
function luc_main_table_latest_spiders()
{
	global $wpdb, $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['spidersrows']) ? $_POST['spidersrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
	?>
	<table class='widefat' >
		<thead>
		<tr>
...
function luc_main_table_latest_spambots()
{
	global $wpdb, $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['spambotsrows']) ? $_POST['spambotsrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
	?>
	<table class='widefat' >
		<thead>
		<tr>
...
function luc_main_table_latest_undefagents()
{
	...
	global $AnalyticStats_Option;
	$table_name = AnalyticStats_TABLE_NAME;
	$querylimit = (isset ($_POST['undefagentsrows']) ? $_POST['undefagentsrows'] : $AnalyticStats_Option['AnalyticStats_Rows_Per_Latest']);
...
function luc_main()
{
	...
	// Overview
    echo "<div class='wrap'>
	<h2>" . __('Overview', 'AnalyticStats') . "</h2>
	...
	<tbody id='the-list'>";
		  ...
		  // SPIDERS ROW
    if ($AnalyticStats_Option['AnalyticStats_Dont_Collect_Spider'] != 'checked')
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php _e(AnalyticStats_PLUGIN_URL) ?>/images/ajax-loader.gif" id="latesthitsLoader" style="display: none;" />
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php _e(AnalyticStats_PLUGIN_URL) ?>/images/ajax-loader.gif" id="latestsearchLoader" style="display: none;" />
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php _e(AnalyticStats_PLUGIN_URL) ?>/images/ajax-loader.gif" id="latestreferrersLoader" style="display: none;" />
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php _e(AnalyticStats_PLUGIN_URL) ?>/images/ajax-loader.gif" id="latestfeedsLoader" style="display: none;" />
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php _e(AnalyticStats_PLUGIN_URL) ?>/images/ajax-loader.gif" id="latestspidersLoader" style="display: none;" />
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php echo AnalyticStats_PLUGIN_URL ?>/images/ajax-loader.gif" id="latestspambotsLoader" style="display: none;" />
...
<!-- tab 8 -->
					...
					<table>
						<tr>
							...
							<td align='right'>
								...
								<img src="<?php _e(AnalyticStats_PLUGIN_URL) ?>/images/ajax-loader.gif" id="latestundefagentsLoader" style="display: none;" />
	...
	<?php
          //echo "</table>";
          echo "<br />";
          echo "&nbsp;<i>". __('AnalyticStats table size', 'AnalyticStats').": <b>" . luc_tablesize($wpdb->prefix. 'AnalyticStats') . "</b></i><br />";
          echo "&nbsp;<i>". __('AnalyticStats current time', 'AnalyticStats').": <b>".current_time('mysql')."</b></i><br />";
          echo "&nbsp;<i>". __('RSS2 url', 'AnalyticStats').": <b>".get_bloginfo('rss2_url').'('.luc_AnalyticStats_extractfeedreq(get_bloginfo('rss2_url')).")</b></i><br />";
          echo "&nbsp;<i>". __('ATOM url', 'AnalyticStats').": <b>".get_bloginfo('atom_url').'('.luc_AnalyticStats_extractfeedreq(get_bloginfo('atom_url')).")</b></i><br />";
          echo "&nbsp;<i>". __('RSS url', 'AnalyticStats').": <b>".get_bloginfo('rss_url').'('.luc_AnalyticStats_extractfeedreq(get_bloginfo('rss_url')).")</b></i><br />";
          echo "&nbsp;<i>". __('COMMENT RSS2 url', 'AnalyticStats').": <b>".get_bloginfo('comments_rss2_url').' ('.luc_AnalyticStats_extractfeedreq(get_bloginfo('comments_rss2_url')).")</b></i><br />";
          echo "&nbsp;<i>". __('COMMENT ATOM url', 'AnalyticStats').": <b>".get_bloginfo('comments_atom_url').' ('.luc_AnalyticStats_extractfeedreq(get_bloginfo('comments_atom_url')).")</b></i><br />";
     luc_AnalyticStats_load_time();
	 ?>
	 </div>
	 <?php
...
function luc_HTML_IMG($key, $type, $showname)
{
	if ($key != '')
	{
		...
		$lines =($type === "country") ?  file(AnalyticStats_PLUGIN_PATH . "/def/domain.dat"):file(AnalyticStats_PLUGIN_PATH . "/def/" . $type . ".dat");
		...
		foreach ($lines as $line_num => $line)
		{
			...
			if (in_array($search, $entry, true))
			{
				...
				$img = (($type === "country") ?
					AnalyticStats_PLUGIN_URL . "images/domain/" . $entry[1] . ".png" :
					AnalyticStats_PLUGIN_URL . "images/" . $type . "/" .  str_replace(" ", "_", str_replace(".", "-", $entry[0])) . ".png");					
				break;
			}
		}
...
{
		if($rk->language != '') 
			         { $lines = file(AnalyticStats_PLUGIN_PATH .'/def/languages.dat');
...
function luc_statistics()
{
          ...
          luc_ValueTable2("date","Top days",5);
		   ...
		   // Search terms
          luc_ValueTable2("search", __('Top search terms', 'AnalyticStats'), 20, "", "", "AND search<>''");
          
          // Top referrer
          luc_ValueTable2("referrer", __('Top referrer', 'AnalyticStats'), 10, "", "", "AND referrer<>'' AND referrer NOT LIKE '%" . get_bloginfo('url') . "%'");
		  
          // O.S.
          luc_ValueTable2("os", __('O.S.', 'AnalyticStats'), 0, "", "", "AND feed='' AND spider='' AND os<>''");
          
          // Browser
          luc_ValueTable2("browser", __('Browser', 'AnalyticStats'), 0, "", "", "AND feed='' AND spider='' AND browser<>''");
          
          // Feeds
          luc_ValueTable2("feed", __('Feeds', 'AnalyticStats'), 5, "", "", "AND feed<>''");
          
          // SE
          luc_ValueTable2("searchengine", __('Search engines', 'AnalyticStats'), 10, "", "", "AND searchengine<>''");
          
          // Countries
          luc_ValueTable2("country", __('Countries ', 'AnalyticStats'), 10, "", "", "AND country<>'' AND spider=''");
          
          // Spider
          luc_ValueTable2("spider", __('Spiders', 'AnalyticStats'), 10, "", "", "AND spider<>''");
...
function luc_AnalyticStats_url_monitoring()
{	global $wpdb, $AnalyticStats_Option;
	$table_name =AnalyticStats_TABLE_NAME;
	$querylimit = 100;
	$pa = luc_page_posts();
...
function luc_spy_bot()
      {   global $wpdb;
		  ...
		  // number of IP or bot by page
		    ...
		    echo "<div class='wrap'><h2>" . __('Bot Spy', 'AnalyticStats') . "</h2>";
          ...
          {  // Bot Spy
			if ($robot <> $rk->spider)  
				...
				<td colspan='2' bgcolor='#dedede'>";
				 ...
				 echo "<IMG style='border:0px;height:16px;align:left;' alt='".$title."' title='".$title."' SRC='" .AnalyticStats_PLUGIN_URL.'/images/spider/'.$img."'>    		
...
function luc_spy_visitors()
      {   global $wpdb;
		  ...
		  // number of IP or bot by page
		  $LIMIT = $AnalyticStats_Option['AnalyticStats_SpyVisitor_IP_Per_Page'];
		  $LIMIT_PROOF = $AnalyticStats_Option['AnalyticStats_SpyVisitor_Visits_Per_IP'];
            if ($LIMIT == 0)
              $LIMIT = 20;
			if ($LIMIT_PROOF == 0)
              $LIMIT_PROOF = 20;
			...
			if ($AnalyticStats_Option['AnalyticStats_Use_GeoIP'] == 'checked' & function_exists('geoip_open'))
			...
			echo "<div class='wrap'><h2>" . __('Visitor Spy', 'AnalyticStats') . "</h2>";
?>
<script>
function ttogle(thediv){
if (document.getElementById(thediv).style.display=="inline") {
...
$sql_posts_pages = "SELECT post_date_gmt,post_title,post_name,post_type 
	...
	$spider = $AnalyticStats_Option['AnalyticStats_Dont_Collect_Spider'];
	if ($spider == '')
	{
...
function luc_AnalyticStats_referrer()
	  {  global $wpdb;
         $table_name = AnalyticStats_TABLE_NAME;
		...
		$NumberArticles = $wpdb->num_rows;
		 ...
		 echo "<div class='wrap'><h2>" . __('Referrer ', 'AnalyticStats')."</div>";
		 luc_print_pp_link($NA,$pa,$action);
             ...
             if ($i <= $NA)
               { // $page is not the last page
	            ...
	            { // Not the current page Hyperlink them
				  ...
				  { if ($action == "overview")
				      echo '<a class="pagination" href="' .admin_url() .'admin.php?page=AnalyticStats/AnalyticStats.php/&pp=' . $i .'">' . $i . '</a> ';
					else  
	                  echo '<a class="pagination" href="' . admin_url() .'admin.php?page=AnalyticStats/action='.$action.'/&pp=' . $i . '&pa=1'.'">' . $i . '</a> ';
					  }
	              else 
	                 { if ($GUIL1 == FALSE) 
	                     {echo "... "; $GUIL1 = TRUE;
              ...
              { // $i is not the last Articles page
               if($j == $pa)  // $i is current page
	              echo " [{$j}] ";
					 ...
					 echo '<a class="pagination" href="' .admin_url() .'admin.php?page=AnalyticStats/action='.$action.'&pa='. $j . '">' . $j . '</a> ';
	                 else 
	                   { if ($GUIL1 == FALSE) 
					      {echo "... "; $GUIL1 = TRUE;}
...
{	global $AnalyticStats_Option;
	...
	if (file_exists(luc_GeoIP_dbname('country')))
	{
		...
		$dbmtime = date_i18n('r', $stat['mtime']);
			...
			$AnalyticStats_Option['AnalyticStats_Use_GeoIP'] = '';
			update_option('AnalyticStats_Option', $AnalyticStats_Option);
		}
	...
	<table class='widefat'>
		...
		<tbody>
			<tr>
				<td><strong>Warning:</strong> GeoIP consumes lot of CPU time, its use is discouraged, do not use it or have permission from your host before activating.
					</br><input type=checkbox name='AnalyticStats_Use_GeoIP' id='AnalyticStats_Use_GeoIP' value='checked'
					<?php echo $AnalyticStats_Option['AnalyticStats_Use_GeoIP'] ?> />
					<label for='AnalyticStats_Use_GeoIP'>Enable (requires MaxMind GeoIP database files to be installed first)</label>
				</br>
				</td><td>
					<?php
					if ($AnalyticStats_Option['AnalyticStats_Use_GeoIP'] == 'checked')
					...
					?>
						...
						<tr>
							...
							<img src="<?php echo AnalyticStats_PLUGIN_URL ?>/images/ajax-loader.gif" id="geoipupdatedbLoader" style="display: none;" />
							<br /><br />
		...
		<thead><tr>
			...
			<form method="post" >
   <tr><td>
       <input type="radio" name="AnalyticStats_locate_IP" value="browser" id="browser" <?php if ($AnalyticStats_Option['AnalyticStats_locate_IP']== 'browser')  echo'checked'; ?>
	   /> <label for="browser">Store the country provided by the browser first, otherwise use GeoIP (default, recommended)</label><br />
       <input type="radio" name="AnalyticStats_locate_IP" value="GeoIP" id="GeoIP" <?php if ($AnalyticStats_Option['AnalyticStats_locate_IP']== 'GeoIP') echo'checked'; ?>
	...
	<table class='widefat'>
		...
		<tbody>
		<tr>
			<td><strong>AnalyticStats GeoIP status:</strong></td>
			<td>
	<?php
	if ($AnalyticStats_Option['AnalyticStats_Use_GeoIP'] == 'checked')
			...
			<table class='widefat'>
				...
				<tr>
					...
					<td>" . $geoip['cn'] . " (" . $geoip['cc'] . ")
						<IMG style='border:0px;height:16px;' alt='$cn' title='$cn' SRC='" . AnalyticStats_PLUGIN_URL . '/images/domain/' . strtolower($geoip['cc']).'.png' . "'></td>
...
{	global $AnalyticStats_Option;
	$AnalyticStats_Use_GeoIP = $AnalyticStats_Option['AnalyticStats_Use_GeoIP'];

	if (file_exists(luc_GeoIP_dbname('country')))
	{
		if ($AnalyticStats_Use_GeoIP == 'checked')
		{
	...
	print "<div class='wrap'><table class='widefat'><thead><tr><th scope='col' style='width:400px;'><h2>$fldtitle</h2></th><th scope='col' style='width:100px;'>".__('Visits','AnalyticStats')."</th><th></th></tr></thead>";
...
function luc_GoogleGeo($title,$size,$data_array) {
	...
	function luc_dropdown_caps($default = false)
      ...
      function luc_AnalyticStats_Abbrevia($s, $c)
      {
          $res = "";
          if (strlen($s) > $c)
              $res = "...";
    ...
    function luc_AnalyticStats_extractfeedreq($url)
    {
		if(!strpos($url, '?') === FALSE)
		{
...
{
	$fp = fopen(AnalyticStats_PLUGIN_PATH . '/def/banips.dat', 'a');
	if ($fp)
		fwrite($fp, "\n" . $ip);
	fclose($fp);
}
		...
		<table class='widefat'>
			...
			<tr>
				...
				<td> <?php _e($geoip['cn'] . " (" . $geoip['cc'] . ")") ?>
					<IMG style='border:0px;height:16px;' alt='$cn' title='$cn' SRC=' <?php _e(AnalyticStats_PLUGIN_URL . "/images/domain/" . strtolower($geoip['cc']) . '.png') ?> '></td>
				<td><strong>Continent Code:</strong></td>
